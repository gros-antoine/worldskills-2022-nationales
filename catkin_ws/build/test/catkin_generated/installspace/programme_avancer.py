...
vitesse_moteurs = 100			#vitesse des moteurs lors du déplacement linéaire du robot
...
def avancer_callback(data):
	...
	while True:
		...
		if moy_poses >= final_pose:
			...
			if current_angle != 0:	
				if current_angle >= ordre + 0.0087: #0.5 deg = 0.0087rad
					moteurs.moteur_droit = 0
				if current_angle <= ordre - 0.0087:
					moteurs.moteur_gauche = 0
			pub_move.publish(moteurs)
		listener_encoders()
		listener_imu()
...
def listener(var):
	rospy.init_node('programme_avancer')
	rospy.Subscriber("pub_avancer", Float32MultiArray, avancer_callback)
	if var:
		rospy.spin()
